<x-app-layout>
    ...
    @section('header-actions')
        <a href="{{ route('orders.show', $order) }}" class="btn-secondary btn-sm">&larr; Back to Order</a>
    @endsection

    <div class="mx-auto max-w-4xl"
         x-data="orderForm({{ $products->toJson() }}, {{ $order->items->toJson() }})">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="tax_rate" name="tax_rate"
                                       ...
                                       value="{{ old('tax_rate', $order->tax_rate) }}"
                    ...
                    <div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        <textarea id="notes" name="notes" rows="6"
                                  class="form-input">{{ old('notes', $order->notes) }}</textarea>
                    </div>
                </div>
    ...
    @push('scripts')
    <script>
        function orderForm(products, existingItems) {
            const buildLines = () => existingItems.length > 0
            ...
            return {
                ...
                discount: {{ old('discount_amount', $order->discount_amount) }},
                subtotal: 0,
                taxAmount: 0,
                total: 0,

                init() { this.computeTotals(); },
